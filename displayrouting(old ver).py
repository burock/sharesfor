def display_exception(exc_ID, lang):
    ...
    doc = db.exceptions.find_one({"exc_ID": exc_ID, "lang": lang})
    if not doc:  # exception undefined
        doc = db.exceptions.find_one({"exc_ID": "EnD", "lang": lang})
    if not doc:
...
def display_routing(otype, oid, thread, thr_list):
    ...
    def tag_section(tags):
        ...
        def tag_add_section():
            body = "<div class='m-2'>"
            body += (
                '<form id="tag-add" action="' + S4S4_BASE + '/add_tag" method="POST">'
            )
            body += hidden_inputs()
            # body += "<div class='input-group'>"
            body += "<div class='input-group-append'>"
            body += (
                '<input type="text" id="tagname" name="tagname" onkeyup="filterFunct('
                + "'obj-list'"
                + ","
                + "'tagname'"
                + ')" placeholder="'
                + xlate_msg("AddTagPlc", LANG)
                + '" autocomplete="off"/>'
            )
            body += '<input type="hidden" id="tagid" name="tagid" />'

            body += (
                "<input type='submit' id='tag-submit' form='tag-add' class='btn-aksiyon' value='"
                + xlate_msg("AddTag", LANG)
                + "'>"
            )
            body += "</div>"
        ...
        def tag_del_section():
            body = "<div class='m-2'>"
            body += (
                '<form id="tag-del" action="' + S4S4_BASE + '/del_tag" method="POST">'
            )
            body += hidden_inputs()
            body += "<div class='input-group-append'>"
            ...
            # body += "<div class='input-group-append'>"
            body += (
                "<input type='submit' form='tag-del' class='btn btn-aksiyon form-control btn-sm btn-aud-add' value='"
                + xlate_msg("RmTag", LANG)
                + "'/>"
            )
            body += "</div>"
            body += "</form>"
        ...
        body += (
            "<a id='toggle-tag-slct' class='akv-link text-sm' onclick='displaySlcts_tags()'>"
            + xlate_msg("ToggleAddRmTags", LANG)
            + "</a>"
        )
    ...
    def aud_selector(audience):
        ...
        def aud_add():
            body = "<div class='m-2'>"
            body += (
                '<form id="aud-add" action="'
                + S4S4_BASE
                + '/add_audience" method="POST">'
            )
            body += hidden_inputs()
            body += "<div class='input-group'>"
            body += (
            ...
            # body += "<input type='submit' form='aud-add' class='btn btn-aksiyon form-control btn-sm btn-aud-add' value='Alıcılara Ekle'/>"
            body += (
                "<input type='submit' form='aud-add' class='btn btn-aksiyon form-control btn-sm btn-aud-add' value='"
                + xlate_msg("AddRcpt", LANG)
                + "'/>"
            )
            body += "</div>"
            body += "</form>"
        ...
        def aud_del():
            body = "<div class='m-2'>"
            body += (
                '<form id="aud-del" action="'
                + S4S4_BASE
                + '/del_audience" method="POST">'
            )
            body += hidden_inputs()
            body += "<div class='input-group'>"
            ...
            body += (
                "<input type='submit' form='aud-del' class='btn btn-aksiyon form-control btn-sm btn-aud-add' value='"
                + xlate_msg("RmRcpt", LANG)
                + "'/>"
            )
            body += "</div>"
            body += "</form>"
        ...
        body += "</div>"
        body += (
            "<a id='toggle-aud-slct' class='akv-link text-sm' onclick='displaySlcts()'>"
            + xlate_msg("ToggleAddRmRcpt", LANG)
            + "</a>"
        )
    ...
    def display_active_thread(msgs):
        ...
        body += (
            "<h4 onclick='showOnModal()'  title='"
            + xlate_msg("ZoomTitle", LANG)
            + "'>+</h4></div>"
        )
        ...
        if msgs:
            for msg in msgs:
                ...
                if msg.get("file"):
                    ...
                    body += (
                        "<a class='btn' href='/download_file?file_id="
                        + str(msg["file"].get("gid"))
                        + "&file_name="
                        + msg["file"].get("fname")
                        + "'>"
                    )
                ...
                body += "</div>"  # row
        body += (
            '<form id="message-add" enctype="multipart/form-data" action="'
            + S4S4_BASE
        ...
        body += (
            ...
            + xlate_msg("EnterMsg", LANG)
            + "' rows='2'></textarea>"
        )
        # body += "<input type='submit' form='message-add' class='btn-aksiyon' value='Gönder' >"
        body += (
            "<input type='submit' form='message-add' class='btn-aksiyon' value='"
            + xlate_msg("BtnSend", LANG)
            + "' >"
        )
        # body += "</div></div></div>"
    ...
    if thr_list:
        ...
        for thr in db.routings.find({"tags.id": [otype, oid]}):
            ...
            if thr["_id"] == thread["thread_id"]:
                ...
                body += (
                    "<li class='list-group-item text-sm'><div class=''>"
                    + aud_list(thr["audience"])
                    + "-"
                    + thr["messages"][-1]["time"].strftime("%d/%m/%y-%H:%M")
                    + ":"
                )
                ...
                if MODAL_DISPLAY:
                    ...
                    body += (
                        "<li class='list-group-item text-sm'><div class=''><a class='akv-link' onclick='"
                        + click
                    ...
                    link += (
                        "&audience=[" + ",".join(thr["audience"]) + "]"
                    )  # send array as string, will parse to array when processing
                    body += (
                        "<li class='list-group-item text-sm'><div class=''><a class='akv-link' href='"
                        + link
                        + "'>"
                        + aud_list(thr["audience"])
                        + "-"
        ...
        if thread["thread_id"] == "0":  # if new thread, display at the bottom
            ...
            if not thread.get("tags"):
                thread["tags"] = [{"id": [otype, oid], "name": object_name(otype, oid)}]
            ...
            body += (
                "<div><a class='akv-link' href='#' onclick='"
                + click
                + "'>"
                + xlate_msg("NewMsg", LANG)
                + "</a></div>"
            )
        else:
            body += (
                "<div><a class='akv-link' href='/routing_form?otype="
                + otype
                + "&oid="
                + str(oid)
                + "&thread_id=0'>"
...
def rtg_list(user):
    ...
    if db.routings.count_documents({"audience": user}) == 0:
        ...
        for thread in db.routings.aggregate(
            [
                {"$match": {"audience": user}},
            ...
            if thread.get("unread") and user in thread.get("unread"):
                body += "<b>"
            body += (
                "<span class='rtg-list-title'>"
                + thread["lastm"]["time"].strftime("%d/%m/%y")
                + " "
                + aud_list(thread["audience"])
                + "</span>"
            )
            if thread["obj_name"]:
...
def object_list_html(inp):
    ...
    for item in lst:
        obj_id_str = json.dumps(
            {"id": [item["otype"], str(item["oid"])], "name": item["oname"]}
        )
        body += (
            "<option value='"
            + obj_id_str
