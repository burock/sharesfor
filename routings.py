...
def get_error_message(exc_ID, lang):
    ...
    db = client.sharesfor
    msg = db.exceptions.find_one({"exc_ID": exc_ID, "lang": lang})
    if not msg:  # exception not defined
        msg = db.exceptions.find_one({"exc_ID": "EnD", "lang": lang})
    return msg
...
def get_user_threads(user):
    db = client.sharesfor
    return db.routings.aggregate(
        [
            {"$match": {"audience.id": user["userid"]}},
...
def handle_attachment(rq_files):
    ...
    with open(
        os.path.join(UPLOADS_FOLDER, filename), "rb"
    ) as f:  # option "rb" is for treating the file as binary
        gid = fs.put(f)
...
def create_rt_thread(otype, oid, thr_params, message, rq_files):
    ...
    try:
        ...
        res = db.routings.insert_one(
            {
                "tags": thr_params["tags"],
                "obj_type": otype,
                "obj_id": oid,
                "obj_name": name,
...
def aud_str2ary(audstr):
    ...
    return (
        audstr.replace("[", "").replace("]", "").replace("'", "").split(",")
    )  # convert str - list
    ...
    if thread["_id"] != 0:
        try:
            ...
            res = db.routings.update_one(
                ...
                {"$pull": {"audience": {"id": aud}, "unread": {"id": aud}}},
            )
            if res.matched_count == 0:
